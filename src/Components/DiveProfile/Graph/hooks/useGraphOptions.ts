...
export default function useGraphOptions<T extends object, X extends NumericalKeys<T>, Y extends NumericalKeys<T>>(
    ...
    return useMemo(() => {
        const [minX, maxX] = [d3.min(data, (d): number => d[options.x]) ?? 0, d3.max(data, (d): number => d[options.x]) ?? 0];
        const [minY, maxY] = [d3.min(data, (d): number => d[options.y]) ?? 0, d3.max(data, (d): number => d[options.y]) ?? 0];
